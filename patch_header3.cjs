...
const startIdx = lines.findIndex(l => l.includes('<!-- Profile Icon (Placed on the far right) -->'));

if(startIdx !== -1) {
  // Check if we already replaced it:
  if (!lines[startIdx + 1].includes('profile-switcher-container')) {
    // We replace the 7 lines
    ...
    lines.splice(startIdx, 7, ...newCode.split('\\n'));
    fs.writeFileSync('src/components/Header.vue', lines.join('\\n'));
