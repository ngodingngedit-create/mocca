...
const targetStr = `        <!-- Profile Icon (Placed on the far right) -->
...
// Try exact replace ignoring \r
const normalizedContent = content.replace(/\\r\\n/g, '\\n');
const normalizedTarget = targetStr.replace(/\\r\\n/g, '\\n');

if (normalizedContent.includes(normalizedTarget)) {
  const result = normalizedContent.replace(normalizedTarget, newCode);
  fs.writeFileSync('src/components/Header.vue', result);
